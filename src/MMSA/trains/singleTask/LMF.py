        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    optimizer.zero_grad()
                    # forward
                    outputs = model(text, audio, vision, text_m, audio_m, vision_m)
                    # compute loss
                    loss = self.criterion(outputs['M_m'], labels) #+ self.criterion(outputs['M'], labels))
                    # backward
                    loss.backward()
                    # update
                    optimizer.step()
            ...
            logger.info(
                ...
                f"{round(train_loss, 4)} {dict_to_str(train_results)}"
            )
            # validation
            ...
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    if return_sample_results:
                        ...
                        preds = outputs["M_m"].cpu().detach().numpy()
                        # test_preds_i = np.argmax(preds, axis=1)
                        sample_results.extend(preds.squeeze())
                    
                    loss = self.criterion(outputs['M_m'], labels)
                    eval_loss += loss.item()
                    y_pred.append(outputs['M_m'].cpu())
                    y_true.append(labels.cpu())
