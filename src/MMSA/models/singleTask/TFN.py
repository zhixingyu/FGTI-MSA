...
class TFN(nn.Module):
    ...
    def forward_c(self, text_x, audio_x, video_x, missing=False):
        '''
        Args:
        ...
        res = {
            f'Feature_t{suffix}': text_h,
            f'Feature_a{suffix}': audio_h,
            f'Feature_v{suffix}': video_h,
            f'Feature_f{suffix}': fusion_tensor,
            f'M{suffix}': output
        }

        return res
    def forward(self, text_x, audio_x, video_x, text_x_m, audio_x_m, video_x_m):
        # o = self.forward_c(text_x, audio_x, video_x, missing=False)
        o_m = self.forward_c(text_x_m, audio_x_m, video_x_m, missing=True)
        return {**o_m}