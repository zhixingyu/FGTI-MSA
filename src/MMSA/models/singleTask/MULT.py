...
class MULT(nn.Module):
    ...
    def forward_o(self, text, audio, video, missing=False):
        if self.use_bert:
        ...
        res = {
            f'Feature_t{suffix}': last_h_l,
            f'Feature_a{suffix}': last_h_a,
            f'Feature_v{suffix}': last_h_v,
            f'Feature_f{suffix}': last_hs,
            f'M{suffix}': output
        }
        return res
    def forward(self, text, audio, vision, text_m, audio_m, vision_m):
        o = self.forward_o(text, audio, vision, missing=False)
        o_m = self.forward_o(text_m, audio_m, vision_m, missing=True)
        return {**o, **o_m}
