...
class LMF(nn.Module):
    ...
    def f(self, text_x, audio_x, video_x, missing=False):
        '''
        Args:
        ...
        res = {
            ...
            f'Feature_f{suffix}': fusion_zy.permute(1, 0, 2).squeeze(),
            f'M{suffix}': output,
        ...
        # }
        return res
    def forward(self, text_x, audio_x, video_x, text_m, audio_m, vision_m):
        # o = self.f(text_x, audio_x, video_x, missing=False)
        m = self.f(text_m, audio_m, vision_m, missing=True)
        return {**m}
