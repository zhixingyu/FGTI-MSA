...
class BERT_MAG(nn.Module):
    ...
    def forward(self, text, audio, video, text_m, audio_m, video_m):
        input_ids, input_mask, segment_ids = text[:,0,:].long(), text[:,1,:].float(), text[:,2,:].long()
        input_ids_m, input_mask_m, segment_ids_m = text_m[:, 0, :].long(), text_m[:, 1, :].float(), text_m[:, 2, :].long()
        output = self.model(
            input_ids,
            video,
            audio,
        ...
        output_m = self.model(
            ...
            video_m,
            audio_m,
            attention_mask=input_mask_m,
            token_type_ids=segment_ids_m,
            position_ids=None,
            head_mask=None,
            inputs_embeds=None,
            labels=None,
