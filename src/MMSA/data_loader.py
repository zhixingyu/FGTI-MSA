    ...
    def __init_mosi(self):
        ...
        if self.args.get('data_missing'):
            ...
            if self.args['need_data_aligned']:
                self.audio_lengths = np.absolute(np.sum(self.text[:,1,:], axis=1, dtype=np.int32))
                self.vision_lengths = np.absolute(np.sum(self.text[:,1,:], axis=1, dtype=np.int32))
